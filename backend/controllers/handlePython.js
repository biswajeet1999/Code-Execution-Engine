...
const handlePython = (code, res) => {
  ...
  fs.writeFileSync(`./code/${user}/temp.py`, code);
  let docker = fs.readFileSync("./docker/python/dockerfile");
  ...
  exec(`docker build -t ${user} ./code/${user}/`, (err, stdout, stderr) => {
    if (err) response.err = "Unable to build docker image...";
      ...
      runContainer.on("exit", () => {
        ...
        fs.unlinkSync(`./code/${user}/temp.py`);
